  ...
  <div class="app-container">
    <el-form ref="updForm" class="form-box" :model="updForm" :rules="rules">
      <el-form-item label="父级菜单" prop="parent_id">
        <el-select v-model="updForm.parent_id" placeholder="请选择父级菜单">
          <el-option
      ...
      <el-form-item label="菜单路径" prop="path">
        <el-input v-model="updForm.path" />
      </el-form-item>

      <el-form-item label="菜单重定向路径" prop="redirect">
        <el-input v-model="updForm.redirect" />
      </el-form-item>

      <el-form-item label="页面组件" prop="component">
        <el-input v-model="updForm.component" />
      </el-form-item>

      <el-form-item label="菜单图标" prop="icon">
        <el-input v-model="updForm.icon" />
      </el-form-item>

      <el-form-item label="菜单排序" prop="sort">
        <el-input v-model="updForm.sort" type="number" />
      </el-form-item>
      ...
      <el-form-item label="隐藏状态" prop="is_hidden">
        <el-radio v-model="updForm.is_hidden" :label="0">显示</el-radio>
        <el-radio v-model="updForm.is_hidden" :label="1">隐藏</el-radio>
      </el-form-item>

      <el-form-item label="启用认证" prop="is_auth">
        <el-radio v-model="updForm.is_auth" :label="0">不启用</el-radio>
        <el-radio v-model="updForm.is_auth" :label="1">启用</el-radio>
      </el-form-item>
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      if (!this.updForm) {
        ...
        this.$store.dispatch('systemMenu/item', this.updForm.id).then(res => {
          this.updForm.parent_id = res.data.parent_id
          this.updForm.name = res.data.name
          this.updForm.title = res.data.title
          this.updForm.path = res.data.path
          this.updForm.redirect = res.data.redirect
          this.updForm.component = res.data.component
          this.updForm.icon = res.data.icon
          this.updForm.sort = res.data.sort
          this.updForm.role_list = res.data.role_list
          this.updForm.is_hidden = res.data.is_hidden
          this.updForm.is_auth = res.data.is_auth

          const checkedRoleList = res.data.role_list.split(',')
          for (let i = 0; i <= checkedRoleList.length; i++) {
            if (checkedRoleList[i]) {
              this.checkedRole.push(parseInt(checkedRoleList[i]))
            }
          }
        })
      }
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.updForm.sort = parseInt(this.updForm.sort)
          this.updForm.role_list = this.checkedRole.join(',')
          this.$store.dispatch('systemMenu/upd', this.updForm)
            .then(res => {
              Message({
                message: res.msg,
                type: 'success',
